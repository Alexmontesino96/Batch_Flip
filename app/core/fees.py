...
def amazon_mfn_fees(
    ...
    """Estima fees de Amazon MFN.

    MFN no incluye fulfillment fee de Amazon. El shipping al cliente se
    modela por separado en el profit engine.
    """
    referral = sale_price * referral_rate
    net = sale_price - referral
    return {
        "marketplace": "amazon_mfn",
        "sale_price": float(sale_price),
        "referral_fee": float(round(referral, 2)),
        "total_fees": float(round(referral, 2)),
        "net_proceeds": float(round(net, 2)),
    }
