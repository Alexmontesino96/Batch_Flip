...
def compute_velocity_from_sales_per_day(
    sales_per_day: float,
    config=None,
) -> VelocityResult:
    """Calcula score de velocidad a partir de una tasa directa de ventas."""
    spd = sales_per_day
...
def compute_velocity(cleaned: CleanedComps, config=None) -> VelocityResult:
    """Compat wrapper para callers que siguen pasando CleanedComps."""
    return compute_velocity_from_sales_per_day(cleaned.sales_per_day, config=config)
